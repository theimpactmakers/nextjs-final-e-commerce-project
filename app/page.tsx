...
export default function Home() {
  return (
    <div className="w-full">
      ...
      <section className="h-96 bg-accent flex items-center justify-center mb-12">
      ...
      <main className="container max-w-7xl px-4 flex flex-col gap-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {['Gratis Versand', 'Sichere Zahlung', 'Expertenberatung', '24/7 Support'].map((feature, index) => (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            {[1, 2, 3, 4].map((item) => (
              <Card key={item} className="overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300">
                <div className="h-48 bg-muted flex items-center justify-center">
                   {/*  */}
                </div>
                <CardHeader>
                  <CardTitle>Produktname {item}</CardTitle>
                  <CardDescription>Kurze Beschreibung des Futters.</CardDescription>
                </CardHeader>
                <CardContent className="flex justify-between items-center pb-4">
                    <span className="text-lg font-bold text-destructive">29,99 €</span>
        ...
        <section className="bg-card p-10 rounded-xl shadow-lg border">
          ...
          <div className="flex justify-center space-x-4 mb-6">
            <Button variant="outline" size="icon" className="rounded-full h-12 w-12 text-lg">FB</Button>
            <Button variant="outline" size="icon" className="rounded-full h-12 w-12 text-lg">IG</Button>
          </div>
