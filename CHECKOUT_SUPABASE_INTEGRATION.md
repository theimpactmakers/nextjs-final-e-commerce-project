...
- **Admin-freundlich**: Methoden können ohne Code-Änderungen aktualisiert werden
- **Gratis-Versand-Logik**: Konfigurierbar pro Versandmethode über `free_shipping_threshold`

#### Verfügbare Versandmethoden:
1. **DHL Standard**: 2,99€, GRATIS ab 50€ (3-5 Tage)
2. **DHL Express**: 7,99€, KEIN Gratis-Versand (1-2 Tage)
...
### 2. Adress-Autofill für angemeldete Benutzer
- **Lädt gespeicherte Adressen automatisch** von Supabase wenn Benutzer authentifiziert ist
- **Intelligentes Fallback**: Standard-Versandadresse → Beliebige Versandadresse → Standard-Adresse → Erste Adresse
- **Smarte Rechnungsadress-Behandlung**: Füllt nur vor, wenn Rechnungsadresse existiert UND von Versandadresse abweicht
- **Benutzerprofil-Integration**: Füllt Name und E-Mail automatisch aus Profil
- **Voll editierbar**: Benutzer können jedes vorausgefüllte Feld ändern
...
- **Kombinierte Anzeige**: Zeigt Gesamt-Ersparnis mit Aufschlüsselung
- **Smarte Nachricht**: 
  - Zeigt nur wenn Ersparnis > 0
  - Schlüsselt nach Typ auf wenn sowohl Aktionen als auch Gratis-Versand zutreffen

Beispiel-Anzeige:
```
💰 Sie sparen insgesamt 8,99 €
   (6,00 € Rabatt + 2,99 € Versand)
```

### 4. Bestellungserstellung in der Datenbank
Vollständige Bestellungserstellung mit allen erforderlichen Feldern:

#### Bestellungsdatensatz (`orders` Tabelle)
- **Bestellnummer**: Automatisch generierte eindeutige ID (Format: `ORD-{timestamp}-{random}`)
- **Benutzer/Gast-Unterstützung**: 
...
- **Adressen (JSONB-Format)**:
  - `billing_address`: Vollständiges Rechnungsadress-Objekt
  - `shipping_address`: Vollständiges Versandadress-Objekt
  - Flexible Struktur unterstützt alle Adressfelder
- **Preisgestaltung**:
  - `subtotal`: Warenkorb-Gesamt
  - `shipping_cost`: **Dynamisch berechnet** aus Datenbank-Versandmethoden
  - `tax_amount`: 19% MwSt-Berechnung
  - `total_amount`: Endgültige Bestellsumme
- **Versand & Zahlung**:
  ...
  - `payment_transaction_id`: Bereit für Stripe Payment Intent
...
- Verknüpft mit Bestellung über `order_id`

### 4. Nach-Bestellungs-Workflow
- **Leert Warenkorb** nach erfolgreicher Bestellungserstellung
- **Weiterleitung** zur Bestellerfolgs-Seite mit Bestellnummer
- **Fehlerbehandlung** mit benutzerfreundlichen Nachrichten

## �️ Datenbank-Schema

### `shipping_methods` Tabelle (NEU)
```sql
- id (uuid, PK)
- code (varchar, unique) - z.B., 'dhl_standard', 'dhl_express'
- name (varchar) - Anzeigename
- description (text, nullable)
- carrier (varchar, nullable) - z.B., 'DHL'
- base_price (numeric) - Basis-Versandkosten
- free_shipping_threshold (numeric, nullable) - Bestellsumme für Gratis-Versand
- estimated_days_min, estimated_days_max (integer, nullable) - Lieferzeit-Bereich
- is_active (boolean, default true)
- display_order (integer) - Sortierreihenfolge in UI
- icon_url (varchar, nullable)
- created_at, updated_at
```

### `payment_methods` Tabelle (NEU)
```sql
- id (uuid, PK)
- code (varchar, unique) - z.B., 'stripe_card', 'klarna', 'sofort'
- name (varchar) - Anzeigename
- description (text, nullable)
- provider (varchar) - 'stripe', 'paypal', etc.
- stripe_payment_method_type (varchar, nullable) - Stripe-spezifischer Typ
- stripe_supported_currencies (text[], nullable) - Unterstützte Währungs-Array
- config (JSONB, nullable) - Anbieter-spezifische Konfiguration
- is_active (boolean, default true)
- display_order (integer) - Sortierreihenfolge in UI
- icon_url (varchar, nullable)
- created_at, updated_at
...
3. Added state for `savedAdressen` and `loadingAdressen`
4. **New useEffect**: Loads user addresses on mount
   - Queries `addresses` Tabelle filtered by `user_id`
...
### `/types/supabase.ts`
**Auto-generated:**
- Includes `shipping_methods` Tabelle types
- Includes `payment_methods` Tabelle types
...
4. Review/edit pre-filled inFormation (fully ediTabelle)
5. **See total savings** including promotions and free shipping
6. Select shipping method - **free standard shipping shown when order ≥ 50€**
7. Select payment method (Card/PayPal/Klarna/Sofort)
8. Accept terms & privacy
9. Klicke "Jetzt kaufen"
...
### With Supabase:
- Queries `shipping_methods` and `payment_methods` Tabelles
- Queries `addresses` Tabelle for user addresses
- Queries `profiles` Tabelle for user data
- Inserts into `orders` Tabelle with dynamic pricing
- Inserts into `order_items` Tabelle
...
- [x] Aktions-Ersparnis calculated correctly
- [x] Versand-Ersparnis calculated correctly
- [x] Total savings display with breakdown
- [x] Adressen load for logged-in users
...
```typescript
// Intelligentes Fallback priority
...
**Status:** ✅ Professional checkout with database-driven methods, auto-fill, and comprehensive savings tracking
